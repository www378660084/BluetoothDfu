...
void PutChar(U8 ch)
{
		...
		(CurrCmdType == BT_CMD_SET_DEG || ch == SLIP_END || dwCharWrite == 200 || (dwCharWrite>=3 && ch == 0xff)))
	{
		...
		{
			log_uart(txTempBuff,dwCharWrite,0);
		}	
		dwCharWrite = 0;
	}
}
...
使用75三代车机音效参数.
*/
static void send_sound_type(int fd)
{
	char buf[5];
	buf[0]='j';buf[1]='a';
	buf[2]='2';buf[3]='\r';buf[4]='\n';	
	write(fd, buf,5);
}
...
int set_bt_dfu(char* port,char* updateFilePath,unsigned int initBaudRate,unsigned int speedBaudRate)
{
	...
	U32	org_baudrate = initBaudRate;
	//U32 high_speed_baudrate = SERIAL_BAUDRATE_NEW;
	U32 high_speed_baudrate = speedBaudRate;
	
	//COMPortString = (U8*)SERIAL_BT_NAME;
	COMPortString = (U8*)port;
	...
	*/
	...
	if(is_already_exist(updateFilePath)){
	//if(is_already_exist(JS_DUF_FILE_NAME_SDCARD) && is_already_exist(JS_DUF_FILE_NAME_UDISK)){
		DLOGD("btdfu no update file!\n");
		ret = -1;
		goto fail;
	}
	...
	//打开DFU升级文件
	dfu_fd = open(updateFilePath, O_RDONLY, 0777);
	//dfu_fd = open(JS_DUF_FILE_NAME_UDISK, O_RDONLY, 0777);
	if (dfu_fd <= 0)
	{
		//dfu_fd = open(JS_DUF_FILE_NAME_SDCARD, O_RDONLY, 0777);
		//if (dfu_fd <= 0)
		//{
		...
		//else
		//{
		//	bt_dfu_file_size = get_file_size(JS_DUF_FILE_NAME_SDCARD);
		//}
	}
	else
	{
		bt_dfu_file_size = get_file_size(updateFilePath);
		//bt_dfu_file_size = get_file_size(JS_DUF_FILE_NAME_UDISK);
	}
